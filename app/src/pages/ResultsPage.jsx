...
export default function ResultsPage() {
    ...
    await getDocs(collection(db, "results")).then((querySnapshot) => {
      ...
      for (const key of visResults.keys()) {
        ...
        const stdDev = Math.sqrt(
          errors.map((x) => Math.pow(x - mean, 2)).reduce((a, b) => a + b) / n,
        );
  ...
  useEffect(() => {
    ...
    const y = d3.scaleLinear().domain(range).range([width, 0]);
  ...
  return (
    <div>
      ...
      <p>Below are the current results for our experiment:</p>
      <p>
        The following graph shows a 95% confidence interval of participant
        accuracy.
      </p>
      <p>
        Accuracy is calculated with the equation: log<sub>2</sub>( | judged
        percent - true percent | + 1/8)
      </p>
      {results.size === 0 ? (
        <div>
          <p style={{ color: "red" }}>
      ...
      <div id="vis"></div>
      {results.size === 0 ? null : (
        <a id="download" download="raw_data.csv" href={rawData}>
          Download Raw Data
        </a>
      )}
    </div>
